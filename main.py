...
def showImageXOR(neural,width,height,size_rec):
    ...
    for x in range(size_rec/2,width,size_rec):
        for y in range(size_rec/2,height,size_rec):
            ...
            w.create_rectangle(top_left[0],top_left[1],bottom_right[0],bottom_right[1], fill=Nn.mapColor(rgb,rgb,rgb))
...
def main():
    ...
    print("Benchmark : "+ str(neural.testDataSet(testSet,targetTestSet)))
